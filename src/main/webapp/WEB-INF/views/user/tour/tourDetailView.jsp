				...
				<table class="table table-bordered" style="border-left:none; border-right:none;">
					<tr>
						...
						<td style="border-top-color: black; border-right:none;">
							<c:choose>
								<c:when test="${tourbean.themecode eq '01'}">숲/산</c:when>
								<c:when test="${tourbean.themecode eq '02'}">해변/바다</c:when>
								<c:when test="${tourbean.themecode eq '03'}">강/호수/계곡</c:when>
								<c:when test="${tourbean.themecode eq '04'}">도심</c:when>
								<c:when test="${tourbean.themecode eq '05'}">섬</c:when>
						</c:choose>
