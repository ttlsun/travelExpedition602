				...
				<td colspan="3" style="text-align: left">
					<select name="orderBy" id="orderBy" onchange="listOrderBy()">
						<option value="regdate desc"
							<c:if test="${map.orderBy eq 'regdate desc' }">selected</c:if>>등록일순</option>
						<option value="moddate desc"
							<c:if test="${map.orderBy eq 'moddate desc' }">selected</c:if>>업데이트순</option>
						<option value="readcount desc"
							<c:if test="${map.orderBy eq 'readcount desc' }">selected</c:if>>조회수 높은순</option>
						<option value="recommend desc"
							<c:if test="${map.orderBy eq 'recommend desc' }">selected</c:if>>추천 많은순</option>
						<option value="steamed desc"
							<c:if test="${map.orderBy eq 'steamed desc' }">selected</c:if>>찜 많은순</option>
						<option value="reviewcount desc"
							<c:if test="${map.orderBy eq 'reviewcount desc' }">selected</c:if>>리뷰 많은순</option>
					</select>
				</td>
			</tr>
		</thead>
		<tbody>
			...
			<c:forEach var="bean" items="${lists}">
				...
				<tr>
					...
					<td colspan="3" class="text-right" style="border-left: none; text-align:end;">
