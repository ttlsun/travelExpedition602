<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>


<!-- kakao api start -->
<script src="http://dmaps.daum.net/map_js_init/postcode.v2.js"></script>
<script type="text/javascript" src="//dapi.kakao.com/v2/maps/sdk.js?appkey=${kakaoKey}"></script>
<!-- kakao api end -->

<div class="" align="center" style="margin-top: 5%; background: #fbfbfb; padding: 40px;">
<hr>
	<span> 
		<a href="agreement" target="_blank"> 이용약관</a> | 
	  	<a href="privacy" target="_blank"> 개인정보 처리방침 </a> | 
	  	<a href="geoLocationAgreement" target="_blank">  위치기반 서비스 이용약관</a>
	</span>
	<span style="white-space: pre-line;">
		ⓒ 여행자들 602호 / 함께한 업체 :  ⓒ <a href="http://www.choongang.co.kr/" target="_blank" title="중앙정보처리학원 페이지로 이동"> 중앙정보처리학원 </a> 
 	</span>
</div>
    
